...
- Auto-generated by content-generator`;
  ...
  // Run deploy directly (don't rely on webhook alone)
  console.log('🚀 Running deployment...');
  execSync(`${PROJECT_ROOT}/deploy.sh`, { stdio: 'inherit', timeout: 300000 });
  
  // Verify deployment with health check
  console.log('🔍 Verifying deployment...');
  const siteUrl = 'https://plastic-surgery-blog.monsoftlabs.com';
  const articleUrl = `${siteUrl}/blog/${topic.slug}/`;
  
  // Wait a moment for any caching to clear
  execSync('sleep 3');
  
  try {
    const result = execSync(`curl -s -o /dev/null -w "%{http_code}" --max-time 10 "${articleUrl}"`, { encoding: 'utf-8' });
    const httpCode = result.trim();
    
    if (httpCode === '200') {
      console.log(`✅ Article live at ${articleUrl}`);
    } else {
      console.log(`⚠️ Article returned HTTP ${httpCode} - may need cache clear`);
    }
  } catch {
    console.log('⚠️ Could not verify article URL (may still be deploying)');
  }
}
